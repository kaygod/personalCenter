 ...
 * 获取某一天的打卡记录
 */
router.post('/get_todo', new Auth().m,genValidator(displineValidate), async (ctx) => {
 ...
 * 提交当天的工作内容
 */
router.post('/clock', new Auth().m,genValidator(displineValidate), async (ctx) => {
  const { user_id } = ctx.auth;
  const { tasks = [],declaration="",is_record } = ctx.data;
  const date = getDate(new Date().getTime());
  ctx.body = await punchClock({ user_id, date, tasks,declaration,is_record });
});
