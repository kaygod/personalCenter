...
exports.getRecord = async (user_id, date) => {
  ...
  if (result.length == 0) {
    const sql_sentence =
      'SELECT a.* FROM records AS a where a.user_id = :user_id AND datediff (a.date,:date) = 0';
    const data = await seq.query(sql_sentence, {
      replacements: {
        user_id,
        date,
      },
    ...
    if (data.length == 0) {
      ...
      const { record_id, date, is_record, declaration } = data[0];
      return {
        declaration,
        record_id,
        date,
